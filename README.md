...
첫 화면의 장르·순위·변화·오래된 게임 분석은 매일 재는 게임들의 같은 기록에서
나온다. 순위 옆의 %는 **그날 잰 것 안에서의 비중**이고, 그 개수는 화면이 직접
적는다 — 여기 숫자를 적어 두면 목록이 늘어난 날 이 문장이 거짓이 된다. Steam
전체 동시접속자는 이 엔드포인트가 주지 않으므로 모르고, 모르는 것을 분모로 쓰지
않는다.
